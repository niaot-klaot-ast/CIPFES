...
for order in range(len(data_oct)):
    ...
    #--------------------------------------------------------------------1
        ...
        for i in range(len(chunk)-1):
            ...
            sig = legendre.legval2d((order+order_init)/normalizing_constant0, 0.5*(chunk[i]+chunk[i+1])/normalizing_constant1, sig_sol2d)
            beta = legendre.legval2d((order+order_init)/normalizing_constant0, 0.5*(chunk[i]+chunk[i+1])/normalizing_constant1, beta_sol2d)
            tck_coeffs1 = np.zeros(shape=len(res_tck_coeffs_array), dtype=float)
            for j in range(len(res_tck_coeffs_array)):
                tck_coeffs1[j] = res_tck_coeffs_array[j](0.5*(chunk[i]+chunk[i+1]), order+order_init)
            ...
            def sgr(x, inten, x0):
                ...
                try:
                    y[mask1] = inten * np.exp(-(abs(x[mask1]-x0)/sig)**beta) + \
                               inten * splev(x[mask1]-x0, tck1)
                except:
                    pass
            ...
            if order+order_init==100 and popt[delta_n+1+1]>2000 and flag==0 and i%2==0:
                ...
                ax1 = plt.Axes(fig, [0.07, 0.42, 0.89, 0.54])
                fig.add_axes(ax1)
                ax2 = plt.Axes(fig, [0.07, 0.12, 0.89, 0.3])
                fig.add_axes(ax2)
                flag = 1
                xb = x1[0]
            ...
            if order+order_init==100 and popt[delta_n+1+1]>2250 and flag==1 and i%2==0:
                ...
                ax2.set_xlabel('pixel position in the principal dispersion direction', fontsize = 14, verticalalignment='top')
                ax2.set_yticks([-0.01, 0, 0.01])
                ax2.set_ylim([-0.015, 0.015])
                ax2.tick_params(which='major', labelsize = 12)
                fig.savefig('zfit1.png', dpi=200)
                # plt.show()
                flag = 2
            ...
            if flag==1:
                ...
                if i%2==0:
                    ...
                    ax2.fill_between(x1, y1=0.015, y2=-0.015, color='k', alpha=0.2)
...
n, bins, patches = ax1.hist(zt[:,4]/zt[:,3], np.linspace(0, np.percentile(zt[:,4]/zt[:,3], 97), 51))
ax1.plot([m1, m1], [0, np.max(n)*1.1], 'k--', lw=2)
ax1.set_ylim([0, np.max(n)*1.1])
ax1.text(0.96, 0.9, r'$\epsilon=0.01$', va='center', ha='right', fontsize=10, transform=ax1.transAxes)
ax1.text(0.96, 0.85, f'Median: {m1:.2f}', va='center', ha='right', fontsize=10, transform=ax1.transAxes)

n, bins, patches = ax2.hist(zt[:,5]/zt[:,3], np.linspace(0, np.percentile(zt[:,5]/zt[:,3], 97), 51))
ax2.plot([m2, m2], [0, np.max(n)*1.1], 'k--', lw=2)
ax2.set_ylim([0, np.max(n)*1.1])
ax2.text(0.96, 0.9, r'$\epsilon=0.008$', va='center', ha='right', fontsize=10, transform=ax2.transAxes)
ax2.text(0.96, 0.85, f'Median: {m2:.2f}', va='center', ha='right', fontsize=10, transform=ax2.transAxes)

n, bins, patches = ax3.hist(zt[:,6]/zt[:,3], np.linspace(0, np.percentile(zt[:,6]/zt[:,3], 97), 51))
ax3.plot([m3, m3], [0, np.max(n)*1.1], 'k--', lw=2)
ax3.set_ylim([0, np.max(n)*1.1])
ax3.text(0.96, 0.9, r'$\epsilon=0.006$', va='center', ha='right', fontsize=10, transform=ax3.transAxes)
ax3.text(0.96, 0.85, f'Median: {m3:.2f}', va='center', ha='right', fontsize=10, transform=ax3.transAxes)

n, bins, patches = ax4.hist(zt[:,7]/zt[:,3], np.linspace(0, np.percentile(zt[:,7]/zt[:,3], 97), 51))
ax4.plot([m4, m4], [0, np.max(n)*1.1], 'k--', lw=2)
ax4.set_ylim([0, np.max(n)*1.1])
ax4.text(0.96, 0.9, r'$\epsilon=0.004$', va='center', ha='right', fontsize=10, transform=ax4.transAxes)
ax4.text(0.96, 0.85, f'Median: {m4:.2f}', va='center', ha='right', fontsize=10, transform=ax4.transAxes)

fig.text(0.5, 0.05, r'$\rm{res}_{\rm{sim}}/\rm{res}_{\rm{obs-rc}}$', va='center', fontsize=12)
fig.text(0.02, 0.53, 'frequency (number of chunks)', va='center', rotation='vertical', fontsize=12)
fig.savefig('zse1.png', dpi=200)
...
n, bins, patches = ax21.hist(zt[mask2,4]/zt[mask2,3], np.linspace(0, np.percentile(zt[mask2,4]/zt[mask2,3], 97), 51), color='b')
ax21.plot([m1, m1], [0, np.max(n)*1.1], 'k--', lw=2)
ax21.set_ylim([0, np.max(n)*1.1])
ax21.text(0.96, 0.9, r'$\epsilon=0.01$', va='center', ha='right', fontsize=10, transform=ax21.transAxes)
ax21.text(0.96, 0.85, f'Median: {m1:.2f}', va='center', ha='right', fontsize=10, transform=ax21.transAxes)

n, bins, patches = ax22.hist(zt[mask2,5]/zt[mask2,3], np.linspace(0, np.percentile(zt[mask2,5]/zt[mask2,3], 97), 51), color='b')
ax22.plot([m2, m2], [0, np.max(n)*1.1], 'k--', lw=2)
ax22.set_ylim([0, np.max(n)*1.1])
ax22.text(0.96, 0.9, r'$\epsilon=0.008$', va='center', ha='right', fontsize=10, transform=ax22.transAxes)
ax22.text(0.96, 0.85, f'Median: {m2:.2f}', va='center', ha='right', fontsize=10, transform=ax22.transAxes)

n, bins, patches = ax23.hist(zt[mask2,6]/zt[mask2,3], np.linspace(0, np.percentile(zt[mask2,6]/zt[mask2,3], 97), 51), color='b')
ax23.plot([m3, m3], [0, np.max(n)*1.1], 'k--', lw=2)
ax23.set_ylim([0, np.max(n)*1.1])
ax23.text(0.96, 0.9, r'$\epsilon=0.006$', va='center', ha='right', fontsize=10, transform=ax23.transAxes)
ax23.text(0.96, 0.85, f'Median: {m3:.2f}', va='center', ha='right', fontsize=10, transform=ax23.transAxes)

n, bins, patches = ax24.hist(zt[mask2,7]/zt[mask2,3], np.linspace(0, np.percentile(zt[mask2,7]/zt[mask2,3], 97), 51), color='b')
ax24.plot([m4, m4], [0, np.max(n)*1.1], 'k--', lw=2)
ax24.set_ylim([0, np.max(n)*1.1])
ax24.text(0.96, 0.9, r'$\epsilon=0.004$', va='center', ha='right', fontsize=10, transform=ax24.transAxes)
...
n, bins, patches = ax31.hist(zt[mask3,4]/zt[mask3,3], np.linspace(0, np.percentile(zt[mask3,4]/zt[mask3,3], 97), 51), color='y')
ax31.plot([m1, m1], [0, np.max(n)*1.1], 'k--', lw=2)
ax31.set_ylim([0, np.max(n)*1.1])
ax31.text(0.96, 0.9, r'$\epsilon=0.01$', va='center', ha='right', fontsize=10, transform=ax31.transAxes)
ax31.text(0.96, 0.85, f'Median: {m1:.2f}', va='center', ha='right', fontsize=10, transform=ax31.transAxes)

n, bins, patches = ax32.hist(zt[mask3,5]/zt[mask3,3], np.linspace(0, np.percentile(zt[mask3,5]/zt[mask3,3], 97), 51), color='y')
ax32.plot([m2, m2], [0, np.max(n)*1.1], 'k--', lw=2)
ax32.set_ylim([0, np.max(n)*1.1])
ax32.text(0.96, 0.9, r'$\epsilon=0.008$', va='center', ha='right', fontsize=10, transform=ax32.transAxes)
ax32.text(0.96, 0.85, f'Median: {m2:.2f}', va='center', ha='right', fontsize=10, transform=ax32.transAxes)

n, bins, patches = ax33.hist(zt[mask3,6]/zt[mask3,3], np.linspace(0, np.percentile(zt[mask3,6]/zt[mask3,3], 97), 51), color='y')
ax33.plot([m3, m3], [0, np.max(n)*1.1], 'k--', lw=2)
ax33.set_ylim([0, np.max(n)*1.1])
ax33.text(0.96, 0.9, r'$\epsilon=0.006$', va='center', ha='right', fontsize=10, transform=ax33.transAxes)
ax33.text(0.96, 0.85, f'Median: {m3:.2f}', va='center', ha='right', fontsize=10, transform=ax33.transAxes)

n, bins, patches = ax34.hist(zt[mask3,7]/zt[mask3,3], np.linspace(0, np.percentile(zt[mask3,7]/zt[mask3,3], 97), 51), color='y')
ax34.plot([m4, m4], [0, np.max(n)*1.1], 'k--', lw=2)
ax34.set_ylim([0, np.max(n)*1.1])
ax34.text(0.96, 0.9, r'$\epsilon=0.004$', va='center', ha='right', fontsize=10, transform=ax34.transAxes)
...
n, bins, patches = ax41.hist(zt[mask4,4]/zt[mask4,3], np.linspace(0, np.percentile(zt[mask4,4]/zt[mask4,3], 97), 51), color='forestgreen')
ax41.plot([m1, m1], [0, np.max(n)*1.1], 'k--', lw=2)
ax41.set_ylim([0, np.max(n)*1.1])
ax41.text(0.96, 0.9, r'$\epsilon=0.01$', va='center', ha='right', fontsize=10, transform=ax41.transAxes)
ax41.text(0.96, 0.85, f'Median: {m1:.2f}', va='center', ha='right', fontsize=10, transform=ax41.transAxes)

n, bins, patches = ax42.hist(zt[mask4,5]/zt[mask4,3], np.linspace(0, np.percentile(zt[mask4,5]/zt[mask4,3], 97), 51), color='forestgreen')
ax42.plot([m2, m2], [0, np.max(n)*1.1], 'k--', lw=2)
ax42.set_ylim([0, np.max(n)*1.1])
ax42.text(0.96, 0.9, r'$\epsilon=0.008$', va='center', ha='right', fontsize=10, transform=ax42.transAxes)
ax42.text(0.96, 0.85, f'Median: {m2:.2f}', va='center', ha='right', fontsize=10, transform=ax42.transAxes)

n, bins, patches = ax43.hist(zt[mask4,6]/zt[mask4,3], np.linspace(0, np.percentile(zt[mask4,6]/zt[mask4,3], 97), 51), color='forestgreen')
ax43.plot([m3, m3], [0, np.max(n)*1.1], 'k--', lw=2)
ax43.set_ylim([0, np.max(n)*1.1])
ax43.text(0.96, 0.9, r'$\epsilon=0.006$', va='center', ha='right', fontsize=10, transform=ax43.transAxes)
ax43.text(0.96, 0.85, f'Median: {m3:.2f}', va='center', ha='right', fontsize=10, transform=ax43.transAxes)

n, bins, patches = ax44.hist(zt[mask4,7]/zt[mask4,3], np.linspace(0, np.percentile(zt[mask4,7]/zt[mask4,3], 97), 51), color='forestgreen')
ax44.plot([m4, m4], [0, np.max(n)*1.1], 'k--', lw=2)
ax44.set_ylim([0, np.max(n)*1.1])
ax44.text(0.96, 0.9, r'$\epsilon=0.004$', va='center', ha='right', fontsize=10, transform=ax44.transAxes)
...
fig.text(0.5, 0.02, r'$\rm{res}_{\rm{sim}}/\rm{res}_{\rm{obs-rc}}$', va='center', fontsize=16)
fig.text(0.015, 0.3, 'frequency (number of chunks)', va='center', rotation='vertical', fontsize=16)
fig.savefig('zse2.png', dpi=200)
...
cbar5.ax.set_xlabel('IP-characterization', fontsize=16)
...
fig5.savefig('zsechunk.png', dpi=200)
